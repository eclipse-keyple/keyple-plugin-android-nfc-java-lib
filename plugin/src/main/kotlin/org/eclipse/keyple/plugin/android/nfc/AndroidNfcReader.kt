...
interface AndroidNfcReader : KeypleReaderExtension {
  ...
  /**
   * Process data from NFC Intent. Can be use to handle NFC Tag received when app is started by nfc
   * detection
   *
  ...
  /**
   * Allows the calling application to specify the delay that the platform will use for performing
   * presence checks on any discovered tag. see @NfcAdapter.EXTRA_READER_PRESENCE_CHECK_DELAY
   *
   * @since 2.0.0
   */
